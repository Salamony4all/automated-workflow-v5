...
def get_session_files():
    ...
    try:
        ...
        # Filter out files that don't exist on disk (sync session state)
        valid_files = []
        for f in uploaded_files:
            # Check if output directory exists (crucial for images)
            file_id = f.get('id')
            if file_id:
                ...
                if os.path.exists(output_dir):
                    ...
                    logger.warning(f"Removing ghost file from session: {file_id} (Output dir missing)")
            else:
                valid_files.append(f)
        ...
        if len(valid_files) < len(uploaded_files):
            ...
            logger.info(f"Cleaned {len(uploaded_files) - len(valid_files)} ghost files from session")
...
def serve_multibudget_outputs(filename):
...
def list_files():
    ...
    try:
        ...
        for f in uploaded_files:
            ...
            # Skip ghost files (check output dir)
            file_id = f.get('id')
            if file_id and session_id:
                output_dir = os.path.join(app.config['OUTPUT_FOLDER'], session_id, file_id)
                if not os.path.exists(output_dir):
                    logger.warning(f"Skipping ghost file in list_files: {file_id}")
                    continue
            
            # Create fresh file_data to return
            file_data = {
                'id': f.get('id'),
                ...
                # Pass extraction_result structure if needed by frontend fallback
                'extraction_result': f.get('extraction_result')
            }

            # Helper to patch HTML paths
            def patch_html_paths(html_content, fid):
                if not html_content or not session_id or not fid:
                    return html_content
                import re
                base_path = f"/outputs/{session_id}/{fid}/imgs/"
                # Regex to find src="filename" but not absolute/http/data
                def repl(match):
                    src = match.group(1)
                    if not src.startswith('/') and not src.startswith('http') and not src.startswith('data:'):
                        return f'src="{base_path}{src}"'
            ...
            if 'stitched_table' in f:
                ...
                html = patch_html_paths(html, f.get('id'))
                file_data['stitched_table'] = {
                    'html': html
                }
            
            # Patch extraction_result HTMLs if present
            if file_data.get('extraction_result') and 'layoutParsingResults' in file_data['extraction_result']:
                # We need to copy extraction_result to avoid mutating session object in memory directly?
                # Actually session object is mutable. But here we modify file_data which is a copy/dict.
                # deepcopy might be safer but extraction_result is large.
                # We'll modify the dict in file_data.
                for page in file_data['extraction_result']['layoutParsingResults']:
                     if 'html' in page:
                         page['html'] = patch_html_paths(page['html'], f.get('id'))
            ...
            if 'costed_data' in f:
                ...
                costed_html = patch_html_paths(costed_html, f.get('id'))
        ...
        if isinstance(result, dict) and 'tables' in result and extraction_method in ['pdfplumber', 'builtin']:
            ...
            for table_idx, table in enumerate(tables_data):
                ...
                if main_header is None:
                    ...
                    for h in headers:
                        h_str = str(h)
                        # Clean Paragraph object reprs if present
                        if '<Paragraph' in h_str:
                            import re
                            match = re.search(r'>([^<]+)$', h_str)
                            if match:
                                h_str = match.group(1).strip()
                            else:
                                # Fallback if regex fails but we know it's garbage
                                h_str = "Column"
                        cleaned_headers.append(h_str)
                        
                    header_cells = ''.join([f'<th>{h}</th>' for h in cleaned_headers])
                ...
                for row in rows:
                    ...
                    cleaned_row = []
                    for cell in row:
                        c_str = str(cell) if cell is not None else ''
                        if '<Paragraph' in c_str:
                             # Clean Paragraph object reprs in data too
                            import re
                            match = re.search(r'>([^<]+)$', c_str)
                            if match:
                                c_str = match.group(1).strip()
                        cleaned_row.append(c_str)
                        
                    row_cells = ''.join([f'<td>{cell}</td>' for cell in cleaned_row])
...
def store_multibudget_table():
    ...
    try:
        ...
        if product_selections_data:
            for selection in product_selections_data:
                if selection.get('brand') and selection.get('category') and selection.get('model'):
                    product_info = {
                        'row_index': selection.get('row_index', len(product_selections)),
                    ...
                    # Use Brand Description and Brand Image from frontend if provided
                    if selection.get('brand_description'):
                        product_info['brand_description'] = selection.get('brand_description')
                    if selection.get('brand_image_html'):
                        product_info['brand_image_html'] = selection.get('brand_image_html')
                    
                    # Get image URL from brand data as fallback
                    ...
                    if product_dropdowns:
                        ...
                        if brand_select and category_select and model_select:
                            ...
                            if brand and category and model:
                                ...
                                # Extract Brand Image and Brand Description from table cells
                                # Find Brand Image cell
                                brand_image_cell = row.find('td', class_='brand-image-cell')
                                if not brand_image_cell:
                                    # Try to find by header index
                                    header_row = table.find('tr')
                                    if header_row:
                                        headers = header_row.find_all(['th', 'td'])
                                        for idx, header in enumerate(headers):
                                            if 'brand image' in header.get_text(strip=True).lower():
                                                if idx < len(cells):
                                                    brand_image_cell = cells[idx]
                                                    break
                                
                                # Extract image URL from Brand Image cell
                                if brand_image_cell:
                                    img_tag = brand_image_cell.find('img')
                                    if img_tag:
                                        image_url = img_tag.get('src', '')
                                        if image_url:
                                            product_info['image_url'] = image_url
                                            product_info['brand_image_html'] = str(brand_image_cell)  # Store HTML for export
                                
                                # Find Brand Description cell
                                brand_desc_cell = row.find('td', class_='brand-description-cell')
                                if not brand_desc_cell:
                                    # Try to find by header index
                                    header_row = table.find('tr')
                                    if header_row:
                                        headers = header_row.find_all(['th', 'td'])
                                        for idx, header in enumerate(headers):
                                            if 'brand description' in header.get_text(strip=True).lower():
                                                if idx < len(cells):
                                                    brand_desc_cell = cells[idx]
                                                    break
                                
                                # Extract description from Brand Description cell
                                if brand_desc_cell:
                                    description = brand_desc_cell.get_text(strip=True)
                                    # Remove placeholder text
                                    if description and 'no description' not in description.lower():
                                        product_info['brand_description'] = description
                                
                                # Fallback: Get image URL from brand data if not found in table
                                if 'image_url' not in product_info:
                                    from utils.image_helper import get_product_image_url
...
@app.route('/api/multibudget/apply-costing/<tier>', methods=['POST'])
def apply_multibudget_costing(tier):
    """Apply costing factors to multi-budget table - separate from regular costing"""
    try:
        data = request.get_json()
        factors = data.get('factors', {})
        table_data = data.get('table_data')  # Table data from frontend
        
        if not table_data:
            return jsonify({'error': 'Table data is required'}), 400
        
        # Use a dedicated multi-budget costing engine
        from utils.costing_engine import CostingEngine
        engine = CostingEngine()
        
        # Apply factors to the table data
        # Convert frontend table_data format to costing engine format
        if 'headers' in table_data and 'rows' in table_data:
            # Already in correct format
            table_format = table_data
        else:
            return jsonify({'error': 'Invalid table data format'}), 400
        ...
        # Store costed data in session for this tier
        if 'multibudget_costed_data' not in session:
            session['multibudget_costed_data'] = {}
        
        session['multibudget_costed_data'][tier] = {
        ...
        return jsonify({
            ...
            'summary': session['multibudget_costed_data'][tier]['summary'],
            'message': 'Costing applied successfully'
        })
    except Exception as e:
        logger.exception('Error applying multibudget costing')
        return jsonify({'error': str(e)}), 500
...
def export_multibudget(tier):
    ...
    try:
        ...
        # Get product selections for this tier
        product_selections = []
        if 'multibudget_tables' in session and tier in session['multibudget_tables']:
            product_selections = session['multibudget_tables'][tier].get('product_selections', [])
        
        # Check if costed data exists for this tier
        if 'multibudget_costed_data' in session and tier in session['multibudget_costed_data']:
            # Use costed data for export
            costed_data = session['multibudget_costed_data'][tier]
            session['uploaded_files'].append({
                'id': file_id,
                'original_name': f'multibudget_{tier}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.html',
                'costed_data': costed_data,  # Use costed data
                'multibudget': True,
                'tier': tier,
                'product_selections': product_selections  # Pass product selections
            })
        elif 'multibudget_tables' in session and tier in session['multibudget_tables']:
            # Fallback to stored table HTML
